...
console.log('\033[00;31mWorker connected to port 7211\033[00m');
...
function setUpDB()
{
  ...
  var fileN = dater + "d.rdb";
  console.log(fileN);

  var cmd2 = "cp " + directToRedis +  "/redis/mydump.rdb " + directToRedis + "/redis/snapshots/" + fileN;
...
sock.on('message', function(msg) {
  ...
  var i = 0;
  var v1 = new Long(message.net[0]["obytes64_2"], message.net[0]["obytes64_1"], true).toNumber();
  var v2 = new Long(message.net[0]["rbytes64_2"], message.net[0]["rbytes64_1"], true).toNumber();
  var v3 = new Long(message.net[0]["opackets_2"], message.net[0]["opackets_1"], true).toNumber();
  var v4 = new Long(message.net[0]["ipackets_2"], message.net[0]["ipackets_1"], true).toNumber();
...
//Some data requires subtraction from previous data, so the first data set is not printed
  if(count > 0) {
    ...
    client.hmset("" + time, "Mem rss", "" + new Long(message.mem[0]["rss_2"], message.mem[0]["rss_1"], true).toNumber());
    client.hmset("" + time, "Mem memcap", "" + new Long(message.mem[0]["physcap_2"], message.mem[0]["physcap_1"], true).toNumber());
    client.hmset("" + time, "Mem swap", "" + new Long(message.mem[0]["swap_2"], message.mem[0]["swap_1"], true).toNumber());
    client.hmset("" + time, "Mem scap", "" + new Long(message.mem[0]["swapcap_2"], message.mem[0]["swapcap_1"], true).toNumber());
    ...
    while(typeof(message.callheat[i]) != "undefined") {
      client.hmset("" + time, "Cal name " + i, "" + message.callheat[i]["name"]);
      client.hmset("" + time, "Cal lowt " + i, "" + new Long(message.callheat[i]["lowt_2"], message.callheat[i]["lowt_1"], true).toNumber());
      client.hmset("" + time, "Cal value " + i, "" + new Long(message.callheat[i]["value_2"], message.callheat[i]["value_1"], true).toNumber());
      i++;
    }
  }
  ...
  while(typeof(message.disk[i]) != "undefined") {
    ...
    var d1 = new Long(message.disk[i]["nread_2"], message.disk[i]["nread_1"], true).toNumber();
    var d2 = new Long(message.disk[i]["nwritten_2"], message.disk[i]["nwritten_1"], true).toNumber();
    var d3 = message.disk[i]["reads"];
    var d4 = message.disk[i]["writes"];
    var d5 = Math.floor(new Long(message.disk[i]["wtime_2"], message.disk[i]["wtime_1"], true).toNumber()/1000) / 1000000;
    var d6 = Math.floor(new Long(message.disk[i]["wlentime_2"], message.disk[i]["wlentime_1"], true).toNumber()/1000)/ 1000000;
