...
//Some data requires subtraction from previous data, so the first data set is not printed
  if(count > 0) {
    ...
    while(typeof(message.callheat[i]) != "undefined") {
      ...
      client.hmset("" + time, "CallHeat lowt " + i, "" + new Long(message.callheat[i]["lowt_2"], message.callheat[i]["lowt_1"], true).toNumber());
      client.hmset("" + time, "CallHeat value " + i, "" + new Long(message.callheat[i]["value_2"], message.callheat[i]["value_1"], true).toNumber());
      i++;
    }
  }
