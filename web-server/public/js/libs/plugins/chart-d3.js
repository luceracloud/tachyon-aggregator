 ...
 * chart-d3.js
 *
 *  A d3 "interface" library that displays
 *  time series data in variously formatted
 *  plots.
 *
 *  Created:      27 Aug 2013
...
function chartd3(canvas, w, h) {
  ...
  if (typeof(d3) == "undefined") {
    console.log("Unable to continue, \"d3\" not found.");
    return null;
  }

  // Variables (local to object)
    ...
    if (this.key_map.indexOf(key) == -1) {
      if (typeof(new_data) == "number") {
        if (type) {
          ...
          this.data[this.key_length] = [0];
        } else {
          this.data[this.key_length] = [new_data];
        }
      } else {
        this.data[this.key_length] = new_data;
      }
  ...
  // Removes all data with the associated key
  this.removeDataKey = removeDataKey;

  function removeDataKey(key) {
    var idx_to_remove = this.key_map.indexOf(key.join());
    this.data[idx_to_remove] = [];
  ...
  function draw_heat_map() {
    ...
    for (var i in this.data) {
      if (this.data_type[this.key_map[i]] == -1) continue;
    ...
    var point_offset = 0;
    for (i in this.data_type) {
      if (this.data_type[i] == -1) { point_offset+=1; }
    }
    vertscale = (this.h - ypad - 20) / (Object.keys(this.data).length-point_offset);

    point_offset = 0;
    for (i in this.data) {
      if (this.data_type[this.key_map[i]] == -1) {
        point_offset += 1;
        continue;
      }
      ...
      for (j in this.data[i]) {
        svg.append("rect")
          ...
          .attr("y", (ypad + 10) + (i-point_offset) * vertscale)
          .attr("width", xscale - 2)
  ...
  // Cycle through display styles for chart
